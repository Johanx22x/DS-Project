...
MenuItem *consultationMenu[] = {
    ...
    new MenuItem(
        2, "Display earliest and latest sunrise of a given year",
        ...
        [](Menu *, Program *ctx) -> CommandCodes {
          ...
          for (Instant *tmp = ctx->instants; tmp != nullptr; tmp = tmp->next) {
            // the constant gmtime calls here are necessary because
            // tmp is always going to change, so there's no reason to cache the
            // result
          ...
          if (max == min) {
            if (max == nullptr) {
              fprintf(stderr, "Couldn't any sunrises for the given year\n");
              return CommandCodes::CONTINUE;
            }
            std::cout << max->str() << "\n";
          } else {
