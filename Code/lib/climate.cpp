...
Climate::Climate(std::string id, double precipitation, double maxTemp,
                 double minTemp, double windSpeed, double humidity,
                 char windDirection, bool itRained, time_t date,
                 time_t startTime, time_t endTime, Rain *rain, Place *place,
                 Person *person) {
  this->id = id;
...
void Climate::show() {
  ...
  while (curr != nullptr) {
    ...
    printf("Associate rain: \n\tID: %s\n\tName: %s\n\tRainfall: %s\n\tRainfall "
           ...
           curr->rain->fmtRainfall().c_str(), curr->rain->rainfall);
    curr = curr->next;
  }
}
...
std::string Climate::str() {
  ...
  out << "Climate ID: " << this->id << "\nClimate location: " << place->name
      ...
      << "\nMaximum temperature: " << this->maxTemp
